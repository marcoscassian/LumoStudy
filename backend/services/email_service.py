...
from config_env import carregar_env, env_bool

carregar_env()

EMAIL_HOST = os.getenv("EMAIL_HOST", "smtp.gmail.com").strip()
EMAIL_PORT = int(os.getenv("EMAIL_PORT", "587"))
EMAIL_USER = os.getenv("EMAIL_USER", "lumostudy934@gmail.com").strip()
...
EMAIL_FROM_NAME = os.getenv("EMAIL_FROM_NAME", "LumoStudy").strip() or "LumoStudy"
...
def email_configurado() -> bool:
    return bool(EMAIL_HOST and EMAIL_PORT and EMAIL_USER and EMAIL_PASSWORD)


def enviar_email_recuperacao(destinatario: str, nome: str, token: str) -> None:
    if not email_configurado():
        raise RuntimeError(
            "SMTP não configurado. Preencha EMAIL_HOST, EMAIL_USER e EMAIL_PASSWORD em backend/.env."
        )

    link = f"{FRONTEND_URL}/redefinir-senha?token={quote(token)}"
    nome_texto = str(nome).strip() or "estudante"
    nome_html = html.escape(nome_texto)
          ...
          <body style="font-family:Arial,sans-serif;color:#211b2d">
            <div style="max-width:560px;margin:auto;padding:28px">
              ...
              <p style="margin:28px 0">
                <a href="{link_html}" style="background:#6e42f5;color:#fff;text-decoration:none;padding:12px 18px;border-radius:10px;font-weight:700">
    ...
    if EMAIL_USE_SSL:
        with smtplib.SMTP_SSL(EMAIL_HOST, EMAIL_PORT, timeout=20, context=contexto) as smtp:
