...
export default function MasteryCard({ image, title, percent, color, general = false }) {
  return (
    <div className={`mastery-card mastery-${color} ${general ? "mastery-general" : ""}`}>
      <div
        className="mastery-ring"
        style={{
          background: `conic-gradient(var(--mastery-color) ${percent}%, #eee 0)`,
        }}
      >
      ...
      <button type="button" className="mastery-btn">
        continuar
      </button>
    </div>
  );
